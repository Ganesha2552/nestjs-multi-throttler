...
import { ThrottlerGuard, ThrottlerStorage, ThrottlerStorageMemoryService, ThrottlerStorageMongoService, ThrottlerStorageRedisService } from '../src';
...
import { mongoURL, mongoDB, mongoCollectionName, mongoExpireAtIndex } from './utility/mongo';
...
async function dropCollectionAndCreateIndex(
  dbUrl: string, dbName: string, collectionName: string, indexField: string): Promise<void> {
...
describe(
  `${adapterName} Throttler ${storageName}`,
  () => {
